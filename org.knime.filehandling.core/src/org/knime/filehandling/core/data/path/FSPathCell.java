...
public final class FSPathCell extends FileStoreCell implements FSPathValue {
    ...
    static final String NULL_PLACEHOLDER = "null_placeholder";
    ...
    private FSPathCell(final FSLocation location) {
        m_location = location;
    }
    ...
    public static final class PathSerializer implements DataCellSerializer<FSPathCell> {
        ...
        public void serialize(final FSPathCell cell, final DataCellDataOutput output) throws IOException {
            output.writeUTF(cell.m_location.getFileSystemType());
            output.writeUTF(cell.m_location.getFileSystemSpecifier().orElse(NULL_PLACEHOLDER));
            output.writeUTF(cell.m_location.getPath());
        }

        @Override
        public FSPathCell deserialize(final DataCellDataInput input) throws IOException {
            final String fileSystemType = input.readUTF();
            final String fileSystemSpecifier = input.readUTF();
            final String path = input.readUTF();
            return new FSPathCell(new FSLocation(fileSystemType,
                fileSystemSpecifier.equals(NULL_PLACEHOLDER) ? null : fileSystemSpecifier, path));
        }
